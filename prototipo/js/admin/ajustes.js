...
function fillAjusteUnidades(){
  document.getElementById('aj-unidad').innerHTML =
...
function syncAjuste(){
  ...
  document.getElementById('aj-err').hidden = true;
}
...
function setAjusteExpensas(){
  const errBox = document.getElementById('aj-err');
  const n = document.getElementById('aj-unidad').value;
  ...
  const habiaAjuste = !!ajustesExpensas[n];
  ajustesExpensas[n] = {tipo, montoCents: Math.round(monto*100), motivo};
  refreshAjusteViews();
  toast(habiaAjuste ? 'Ajuste de expensas reemplazado' : 'Ajuste de expensas aplicado');
}
...
function confirmQuitarAjuste(n){
  ...
  openModal('Quitar ajuste',
    ...
    [{label:'Cancelar', cls:'btn-ghost', fn:closeModal},
     {label:'Quitar ajuste', cls:'btn-danger', fn:()=>{ quitarAjuste(n); closeModal(); }}]);
}
function quitarAjuste(n){
  delete ajustesExpensas[n];
  refreshAjusteViews();
  syncAjuste();
  toast('Ajuste quitado');
}
function refreshAjusteViews(){
  renderAjustes();
  renderCobranzas();
  renderAccounts();
  if(selectedIdx!==null) selectUnit(selectedIdx);
}
