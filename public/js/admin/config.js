...
function totalExpensasPct(excludeN) {
  return Object.keys(expensasPct).reduce(
    (acc, n) => (n === excludeN ? acc : acc + Math.round(expensasPct[n] * 100)), 0,
  ) / 100;
}

/* El servidor revalida todo esto; acá solo evitamos el ida y vuelta. */
...
function fillConfigUnidades() {
  const sel = document.getElementById('cf-unidad');
...
async function setPorcentajeExpensa() {
  const n = document.getElementById('cf-unidad').value;
  const pct = parsePct(document.getElementById('cf-pct').value);
  const v = validarPorcentajeExpensa(n, pct);
  if (!v.ok) return mostrarError('cf-err', v.msg);

  const r = await apiEnviar('POST', `/api/locales/${n}/porcentaje`, { pct });
  if (!r.ok) return mostrarError('cf-err', r.msg);

  mostrarError('cf-err', null);
...
function renderConfig() {
  ...
  document.getElementById('cf-total').textContent =
    `Total asignado: ${fmtPct(total)} · Disponible: ${fmtPct(100 - total)}`;
